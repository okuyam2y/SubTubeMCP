...
- **機能別フォールバック**: 動画メタデータは yt-dlp → ytdl-core、字幕は youtube-transcript → yt-dlp
...
- `YOUTUBE_API_KEY`: 検索・チャンネル・トレンド・コメント機能に必要（なくても基本機能は動作）
...
**レスポンスの切断情報:**
- `isTruncated`: `fullText` またはセグメントのどちらかが切断された場合に `true`
- `segmentTruncated`: セグメント配列が `maxSegments` により切断されたか
- `fullTextTruncated`: `fullText` が50,000文字上限で切断されたか
- `fullTextOriginalLength` / `fullTextReturnedLength`: 切断前と返却対象の文字数
- `fullTextRange`: 返却した元テキスト範囲（`start` / `endExclusive`）
- `message`: 切断範囲と、残りをsegmentsまたは `mode: "full"` + `maxSegments` で参照する方法
...
`dist/index.js` をビルドして実際に stdio 接続し、MCP の初期化、9ツールの一覧取得、1ツールの呼び出し経路を自動判定します。この試験は YouTube API キー、ネットワーク、yt-dlp の実通信を必要としません。外部サービスを含む統合試験には `bash tests/run-tests.sh` を使います。
...
### v1.7.0
- 🔖 バージョン表記を統一（package.json / package-lock.json / MCPサーバー識別子を一致）
- 🔒 依存パッケージを更新し既知の脆弱性を解消（googleapis 173系へ更新）

### v1.6.0
- 🛡️ コメントスクリーニング機能追加（スパム・ノイズ自動除外）
- 📊 フィルタリングで約25%のコメント削減、18%のデータ転送量削減
- ⚙️ noFilterオプションで全コメント取得も可能
